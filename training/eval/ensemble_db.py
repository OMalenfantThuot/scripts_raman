...
def build_parser():
    ...
    parser.add_argument("--models", help="Paths to the trained models.", default=[], nargs="*")
...
def main(args):
    ...
    calculator = mlcalcdriver.calculators.EnsembleCalculator(
        args.models, device=device
    )
    ...
    for l1, l2, l3 in zip(answers, results, stds):
        an, re, std = np.array(l1).flatten(), np.concatenate(l2).flatten(), np.concatenate(l3).flatten()
